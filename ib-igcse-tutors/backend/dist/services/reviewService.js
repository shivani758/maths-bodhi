...
function buildReviewPayload(payload, existing) {
    return {
        id: existing?.id,
...
async function updateReviewRow(id, payload) {
    const result = await query(`
      UPDATE reviews
      SET
        source_id = $1,
...
export async function listReviews() {
    const result = await query("SELECT * FROM reviews ORDER BY order_index ASC, updated_at DESC");
    return result.rows.map((row) => serializeReview(toReview(row)));
}
export async function getReviewById(id) {
    const review = await findContentRowById("reviews", id);
    if (!review) {
...
export async function deleteReview(id) {
    const review = await deleteContentRowById("reviews", id);
    if (!review) {
        throw new ApiError(404, "Review not found.", { code: "REVIEW_NOT_FOUND" });
    }
    return serializeReview(toReview(review));
}
export async function upsertReviewBySourceId(payload) {
    const existing = await query("SELECT * FROM reviews WHERE source_id = $1 LIMIT 1", [
        payload.sourceId,
    ]);
    if (existing.rows[0]) {
